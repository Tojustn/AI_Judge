...
const AddJudgeCard = ({changeState}: AddJudgeCardProps) => {
  ...
  const [targetModelName, setTargetModelName] = useState<TargetModelName>("gpt-4")

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try{
        const judge: Judge= {name: name, rubric: instructions, targetModelName: targetModelName, active: true, id: generateId(), createdAt: Date.now()}
    ...
    <div className="w-full">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <div className="flex flex-col">
          <label className="mb-1 text-gray-700 dark:text-gray-300">Judge Name</label>
          <input
            type="text"
            required
            value={name}
        ...
        <div className="flex flex-col">
          <label className="mb-1 text-gray-700 dark:text-gray-300">Instructions</label>
          <textarea
            value={instructions}
        ...
        <RadioComponent chosen={targetModelName} onChange={onModelChange}></RadioComponent>
          <button
            type="submit"
